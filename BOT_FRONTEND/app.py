...
# 3D Avatar Toggle Button
if "show_avatar" not in st.session_state:
    st.session_state.show_avatar = False

col1, col2 = st.columns([3, 1])
with col2:
    if st.button("🎙️ Talk to 3D Avatar" if not st.session_state.show_avatar else "❌ Close 3D Avatar", use_container_width=True):
        st.session_state.show_avatar = not st.session_state.show_avatar
        st.rerun()

# Embed 3D Avatar Interface
if st.session_state.show_avatar:
    with open("assets/avatar.html", "r", encoding="utf-8") as f:
        html_content = f.read()
    
    # Replace relative paths with absolute FastAPI backend URL for Streamlit embedding
    backend_url = os.getenv("API_HOST", "http://localhost:8000")
    html_content = html_content.replace("'models/avatar.glb'", f"'{backend_url}/assets/models/avatar.glb'")
    html_content = html_content.replace("fetch('/api/voice'", f"fetch('{backend_url}/api/voice'")
    
    st.markdown(f"*(If microphone doesn't work inside this block, <a href='{backend_url}/assets/avatar.html' target='_blank'>click here to open the Avatar in a new tab</a>)*", unsafe_allow_html=True)
